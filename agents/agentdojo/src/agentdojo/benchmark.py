...
def run_task_with_injection_tasks(
    ...
    total_injection_tasks = len(injection_tasks_to_run) if hasattr(injection_tasks_to_run, '__len__') else '?'
    for inj_idx, injection_task_id in enumerate(injection_tasks_to_run):
        injection_task = suite.get_injection_task_by_id(injection_task_id)
        print(f"  [INJECTION] Task {inj_idx + 1}/{total_injection_tasks}: {injection_task_id}", flush=True)
        ...
        # Always rerun - skip caching logic
...
def benchmark_suite_with_injections(
    ...
    Args:
        ...
        sampled_pairs: If set, only run these (user_task_id, injection_task_id) pairs.
    """
    ...
    # If sampled_pairs provided, filter user_tasks and group injection tasks per user
    if sampled_pairs is not None:
        pair_map = defaultdict(list)
        for ut_id, inj_id in sampled_pairs:
            pair_map[ut_id].append(inj_id)
        user_tasks_to_run = [suite.get_user_task_by_id(uid) for uid in pair_map]
    else:
        pair_map = None

    total_sampled = len(sampled_pairs) if sampled_pairs is not None else None
    if total_sampled is not None:
        print(f"[SAMPLE] Running {total_sampled} sampled pairs for suite '{suite.name}'", flush=True)
    sample_counter = 0
    print(f"Total_user_tasks: {len(list(user_tasks_to_run))}", flush=True)
    total_user_tasks = len(list(user_tasks_to_run)) if hasattr(user_tasks_to_run, '__len__') else '?'
    for task_idx, user_task in enumerate(user_tasks_to_run):
        print(f"\n{'='*60}", flush=True)
        print(f"[BENCHMARK] Running user task {task_idx + 1}/{total_user_tasks}: {user_task.ID}", flush=True)
        print(f"{'='*60}", flush=True)
        inj_tasks_for_this_user = pair_map[user_task.ID] if pair_map else injection_tasks
        if total_sampled is not None:
            for inj_id in inj_tasks_for_this_user:
                sample_counter += 1
                print(f"  [SAMPLE {sample_counter}/{total_sampled}] {user_task.ID} x {inj_id}", flush=True)
