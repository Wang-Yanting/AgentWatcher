...
def recursive_defense(obj, defense, target_inst: str, defense_name: str = None, monitor_llm: str = None, target_model_response: str = None):
    ...
    For agentwatcher / datasentinel: treat entire object as string and apply defense once (no recursion).
    ...
    Args:
        ...
        defense_name: Name of the defense; must be set for agentwatcher / datasentinel whole-context behavior.
    ...
    Returns:
        Tuple of (defended/cleaned object, total_defense_time: float for this recursion)
    """
    if defense_name == "agentwatcher":
        ...
        t = float(result.get("time", 0)) if result.get("time") is not None else 0.0
        if result.get("detect_flag"):
            return "", t
        elif "cleaned_context" in result:
            return str(result["cleaned_context"]), t
        else:
            return context_str, t

    if defense_name == "datasentinel":
        if isinstance(obj, (dict, list)):
            context_str = json.dumps(obj, indent=2, ensure_ascii=False)
        else:
            context_str = str(obj)
        result = defense.execute(target_inst=target_inst, context=context_str)
        t = float(result.get("time", 0)) if result.get("time") is not None else 0.0
        if result.get("detect_flag"):
            return "", t
        if "cleaned_context" in result and result["cleaned_context"] is not None:
            cleaned = result["cleaned_context"]
            if isinstance(cleaned, (dict, list)):
                return json.dumps(cleaned, indent=2, ensure_ascii=False), t
            return str(cleaned), t
        return context_str, t
    ...
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            c, dt = recursive_defense(v, defense, target_inst, defense_name, monitor_llm, target_model_response)
            cleaned_dict[k] = c
            total_t += dt
        return cleaned_dict, total_t
        ...
        if "cleaned_context" in result and defense_name not in ("agentwatcher", "datasentinel"):
            cleaned = result["cleaned_context"]
            if isinstance(cleaned, (dict, list)):
                return json.dumps(cleaned, indent=2, ensure_ascii=False), t
            return str(cleaned), t
    ...
    def query(
        ...
        if msg["role"] == "tool" and "content" in msg:
            try:
                ...
                json_data = parse(raw_data)
                monitor_llm = os.environ.get("PIARENA_MONITOR_LLM")
                cleaned, defense_time = recursive_defense(
                    json_data,
                    defense=defense,
                    target_inst=user_instruction,
                ...
                _AGENTDOJO_DEFENSE_TIMES.append(defense_time)
                cleaned_str = json.dumps(cleaned, indent=2, ensure_ascii=False)
...
class PIMonitorLLMDefenseAdapter(BasePipelineElement):
    ...
    For agentwatcher, set PIARENA_MONITOR_LLM to the monitor LLM path or id.
    """
