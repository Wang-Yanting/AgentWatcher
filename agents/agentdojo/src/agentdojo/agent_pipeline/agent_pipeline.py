...
def get_llm(provider: str, model: str, model_id: str | None, tool_delimiter: str) -> BasePipelineElement:
    if provider == "openai":
        # Always use OpenAI directly (not Azure)
        ...
        model = anthropic_model_map.get(model, model)
        ...
        if model_id is None:
            raise ValueError("model_id is required for transformers provider")
...
class AgentPipeline(BasePipelineElement):
    ...
    def from_config(cls, config: PipelineConfig) -> Self:
        ...
        if config.defense == "piarena":
            ...
            if piarena_defense_name == "agentwatcher":
                from agentdojo.agent_pipeline.piarena_defense_adapter import PIMonitorLLMDefenseAdapter
                defense_element = PIMonitorLLMDefenseAdapter()
                tools_loop = ToolsExecutionLoop([ToolsExecutor(tool_output_formatter), llm, defense_element])
            else:
