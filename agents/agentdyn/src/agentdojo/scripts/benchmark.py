...
def show_results(suite_name: str, results: SuiteResults, show_security_results: bool, defense: str | None = None):
    ...
    if defense == "piarena":
        try:
            ...
            if _AGENTDOJO_DEFENSE_TIMES:
                avg_time = sum(_AGENTDOJO_DEFENSE_TIMES) / len(_AGENTDOJO_DEFENSE_TIMES)
                print(f"Average defense time: {avg_time:.2f} s ({len(_AGENTDOJO_DEFENSE_TIMES)} calls, excludes backend LLM)")
        except ImportError:
            pass
...
@click.option(
    ...
    type=int,
    default=None,
    help="Randomly sample this many (user_task, injection_task) pairs across all suites under attack. If None, all pairs are run.",
)
def main(
    suites: tuple[str, ...],
    model: ModelsEnum,
    ...
    per_suite_pairs: dict[str, list[tuple[str, str]]] = {}
