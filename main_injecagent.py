...
Usage:
    ...
    python main_injecagent.py --model meta-llama/Llama-3.1-8B-Instruct --defense datasentinel

    # GPT-4o backend
    python main_injecagent.py --model gpt-4o-2024-05-13 --defense promptarmor
...
from src.utils import save_json, setup_seeds, nice_print
...
def apply_defense_to_string(defense, user_instruction: str, text: str, 
                            monitor_llm: str = None, llm = None, target_model_response: str = None,
                            context_for_attribution: str = None) -> str:
    """
    ...
    - PIVerifier: detection-based, empty string if detected (pass target_model_response)
    
    For AgentWatcher (agentwatcher) with multi-step (e.g., InjecAgent): pass context_for_attribution as the
    concatenation of all previous step contexts. Attribution runs on that full context,
    but the return value is always `text` (current step's context).
    """
    # Handle AgentWatcher (agentwatcher / legacy names) specially
    if defense == "agentwatcher":
        from src.defenses.agentwatcher.defense_agentwatcher import agentwatcher
        # Use concatenated previous contexts for attribution when provided (multi-step)
        watcher_context = context_for_attribution if context_for_attribution is not None else text
        result = agentwatcher(
        ...
        return text  # Always return current step's context, not full concatenated
...
def apply_defense(defense, user_instruction: str, context: str, 
                  monitor_llm: str = None, llm = None, target_model_response: str = None,
                  context_for_attribution: str = None) -> str:
    """
    ...
    - Sanitization (DataFilter, PISanitizer): return cleaned_context
    - Hybrid (PromptArmor, PromptLocate): return cleaned_context
    - AgentWatcher (agentwatcher): detection-based (pass target_model_response)
      For multi-step: pass context_for_attribution = concatenation of all previous contexts
    - None: return original context
    """
    if defense is None:
        return context

    # For AgentWatcher, apply to the whole context string (not recursive)
    if defense == "agentwatcher":
        return apply_defense_to_string(
            defense, user_instruction, context, monitor_llm, llm, target_model_response,
            context_for_attribution=context_for_attribution
        )
...
def main(args):
    ...
    if args.defense == "none":
        ...
        print(f"[Defense] Loaded {args.defense} (AgentWatcher) with monitor_llm={args.monitor_llm}")
    else:
    ...
    for setting in ["base", "enhanced"]:
        ...
        # Random sampling if data_num specified
        if args.data_num > 0 and args.data_num < len(test_cases_dh):
            import random
            random.seed(args.seed)
            indices = random.sample(range(len(test_cases_dh)), args.data_num)
            indices.sort()
            test_cases_dh = [test_cases_dh[i] for i in indices]
            print(f"[DH] Randomly selected {args.data_num} samples (seed={args.seed})")
        ...
        for i, case in enumerate(tqdm(test_cases_dh, desc=f"DH-{setting}")):
            ...
            if defense != "agentwatcher":
                ...
                # For agentwatcher: query first, then detect
                try:
                    agent_output = query_agent(model, case, case["Tool Response"], tool_dict)
                except Exception as e:
                    print(f"\n[Error] Case {i}: {e}")
                    agent_output = None
                ...
                filtered_response = apply_defense(
                    ...
                    monitor_llm=args.monitor_llm,
                    llm=model,
                    target_model_response=agent_output
                )
        ...
        # Random sampling if data_num specified
        if args.data_num > 0 and args.data_num < len(test_cases_ds):
            import random
            random.seed(args.seed + 1)  # Different seed for DS
            indices = random.sample(range(len(test_cases_ds)), args.data_num)
            indices.sort()
            test_cases_ds = [test_cases_ds[i] for i in indices]
            print(f"[DS] Randomly selected {args.data_num} samples (seed={args.seed + 1})")
            ...
            if eval_result["eval"] == "succ":
                ...
                if attacker_tool_response:
                    ...
                    if defense != "agentwatcher":
                        ...
                        try:
                            agent_output_step2 = query_agent_step2(
                                model, case, eval_result, attacker_tool_response, tool_dict
                            )
                        except Exception as e:
                            print(f"\n[Error] Case {i} Step2: {e}")
                            agent_output_step2 = None

                        # Multi-step: context for attribution = concatenation of all previous contexts
                        context_for_attribution = case["Tool Response"] + "\n\n" + attacker_tool_response
                        filtered_attacker_response = apply_defense(
                            defense,
                            user_instruction=case["User Instruction"],
                            context=attacker_tool_response,
                            monitor_llm=args.monitor_llm,
                            llm=model,
                            target_model_response=agent_output_step2,
                            context_for_attribution=context_for_attribution
                        )

                        nice_print(f"[DS] Case {i} Filtered Attacker Response (After Defense): {filtered_attacker_response}")
                        try:
                            agent_output_step2 = query_agent(model, case, filtered_attacker_response, tool_dict)
                        except Exception as e:
                            print(f"\n[Error] Case {i}: {e}")
                            agent_output_step2 = None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--monitor_llm", type=str, default="SecureLLMSys/AgentWatcher-Qwen3-4B-Instruct-2507",
                        help="Monitor LLM for AgentWatcher defense")
    ...
    parser.add_argument("--data_num", type=int, default=100,
                        help="Number of samples to randomly select (-1 for all)")
