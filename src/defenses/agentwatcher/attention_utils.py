...
def get_layer_attention_weights(
    ...
    if model_type in ("llama", "qwen2", "qwen3", "qwen1.5", "gemma3", "glm"):
    ...
    # Use chunked computation for long sequences to avoid OOM
    num_query_tokens = query_states.shape[2]
    num_key_tokens = key_states.shape[2]
    total_elements = num_query_tokens * num_key_tokens
    
    # For long sequences, compute attention in chunks and move to CPU to reduce peak GPU memory
    # Lower threshold (20M) helps when vLLM + attribution share GPU and memory fragments over batches
    if total_elements > 20_000_000:  # ~20M elements threshold
        # Determine chunk size - smaller for longer sequences
        if total_elements > 500_000_000:
            chunk_size = max(16, num_query_tokens // 128)
        elif total_elements > 100_000_000:
            chunk_size = max(32, num_query_tokens // 64)
        else:
            chunk_size = max(64, num_query_tokens // 32)
        
        attn_weights_list = []
        for i in range(0, num_query_tokens, chunk_size):
            end_i = min(i + chunk_size, num_query_tokens)
            q_chunk = query_states[:, :, i:end_i]
            mask_chunk = causal_mask[:, :, i:end_i]
            
            # Compute attention for this chunk
            chunk_attn = torch.matmul(q_chunk, key_states.transpose(2, 3)) / math.sqrt(head_dim)
            chunk_attn = chunk_attn + mask_chunk
            dtype = chunk_attn.dtype
            chunk_attn = torch.softmax(chunk_attn, dim=-1, dtype=torch.float32).to(dtype)
            
            # Move to CPU immediately to free GPU memory
            attn_weights_list.append(chunk_attn.cpu())
            del q_chunk, mask_chunk, chunk_attn
            
            # Clear cache every few chunks
            if (i // chunk_size) % 4 == 3:
                torch.cuda.empty_cache()
        
        # Concatenate on CPU then move back
        attn_weights = torch.cat(attn_weights_list, dim=2).to(query_states.device)
...
def get_attention_weights_one_layer(
    ...
    with torch.no_grad():
        ...
        # Directly compute weights without pre-allocating (the pre-allocation was wasteful)
